...
object OrderApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
      * 从Kafka里面获取到Canal采集好的增量。
      */
    val inputDstream: InputDStream[ConsumerRecord[String, String]] = MyKafkaUtil.getKafkaStream(
      GmallConstant.KAFKA_TOPIC_ORDER,
      ssc)

    /**
      * 把从Kafka采集到的新增的json格式的订单日志，转换成订单日志对象，加时间戳，同时进行一定的数据清洗工作
      */
    ...
    /**
      * 把经过转换和数据清洗过的，新增的订单日志对象，保存到Phoenix里面
      */
    orderInfoDstream.foreachRDD { rdd =>
      rdd.saveToPhoenix(
        "GMALL0826_ORDER_INFO",
